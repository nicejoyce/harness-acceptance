...
test('exports only allowlisted files with an injected independent CODEOWNER', async () => {
  ...
  await mkdir(path.join(source, 'harness'), { recursive: true });
  ...
  assert.equal(git('add', '.gitignore', 'package.json', 'CODEOWNERS.template', '.github', 'harness', 'harness-cli', 'docs').status, 0);
