...
test('rejects an approver who is authorized for a different role', async () => {
  const bundle = await loadContracts(path.resolve('harness'));
  const diagnostics = await validateApprovals(bundle, [approval({ approver: 'security' })], context);
...
test('rejects malformed timestamps and an expiry that precedes approval', async () => {
  const bundle = await loadContracts(path.resolve('harness'));
  const base = approval({ approved_at: 'not-a-date', expires_at: 'also-not-a-date' });
...
test('rejects approval provenance that does not match the execution context', async () => {
  const bundle = await loadContracts(path.resolve('harness'));
  const diagnostics = await validateApprovals(bundle, [approval({ commit_sha: 'b'.repeat(40), repository: 'attacker/repo', pull_request: 7 })], context);
...
test('matches authorized GitHub identities case-insensitively', async () => {
  ...
  assert.deepEqual(await validateApprovals(bundle, [approval({ approver: 'engineering' })], context), []);
});
