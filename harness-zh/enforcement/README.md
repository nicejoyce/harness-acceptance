...
P0 的真实执行入口是 `.github/workflows/harness.yml`。仓库管理员只应将 `harness-final` 配置为 Harness 必需 Check，并要求非作者 CODEOWNER Review。`prepare-context` 从受保护 base 生成 canonical Plan 和审批；无凭证的 `harness` 矩阵 Job 执行项目命令并输出不可信 Evidence；`harness-final` 重新获取当前 Review、验证所有上下文绑定 Evidence、重算结果、使用 Ed25519 签名并发布稳定 Check。
